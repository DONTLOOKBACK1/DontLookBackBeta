...
@export var invincibility_duration: float = 0.5 # Segundos de inmunidad
...
@export var wall_kick_extra_force: float = 150.0 # Push extra para wall kick
...
func take_damage(amount: int):
	# Si ya somos invencibles, no recibir más daño
	if is_invincible:
		return
	...
	# Reproducir sonido y efecto visual
	damage_sound_player.play()
	sprite.modulate = Color.RED # Flash rojo inicial
...
func _on_blink_timer_timeout():
	# Alterna la visibilidad del sprite para crear el parpadeo
	sprite.visible = not sprite.visible

func _on_invincibility_timeout():
	# Se acabó el tiempo de inmunidad
	is_invincible = false
	
	# Detener el parpadeo
	blink_timer.stop()
	
	# Restaurar el estado visual normal
	sprite.visible = true
	sprite.modulate = Color.WHITE # Color.WHITE es el modulador por defecto (sin tinte)
...
# --- PAREDES ---
func get_wall_normal_custom() -> Vector2:
...
func handle_jump():
	...
	# --- PRIORIDAD 1: Wall Jump / Wall Kick ---
	if is_wall_sliding:
		var wall_normal = get_wall_normal()
		var direction = Input.get_axis("ui_left", "ui_right")
		...
		# Si mantienes movimiento hacia la pared → salto leve
		if sign(direction) == -sign(wall_normal.x):
			velocity.x = wall_normal.x * wall_jump_horizontal_velocity
		else:
			# Wall Kick estilo Hollow Knight (más fuerza)
	...
	# --- PRIORIDAD 2: Salto Normal ---
	if is_on_floor():
		velocity.y = jump_velocity

# --- MOVIMIENTO HORIZONTAL ---
func handle_horizontal_movement(delta: float):
	# No dejar mover durante Wall Kick
	if is_in_kick:
...
func update_animation():
	...
	if current in ["attack", "crouch_down", "crouch_up"]:
		return
	...
	if is_attacking:
		...
		if is_crouching:
			new_anim = "crouch_walk" if velocity.x != 0 else "crouch_idle"
		else:
			new_anim = "run" if velocity.x != 0 else "default"

	else:
		new_anim = "jump" if velocity.y < 0 else "fall"

	if new_anim != "" and new_anim != current:
